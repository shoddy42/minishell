...
/*   Updated: 2022/10/17 18:43:28 by wkonings      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int		child_p_2(t_command *cmd, t_minishell *shell)
{
	return (1);
}
...
int		child_p_1(t_command *cmd, t_minishell *shell)
{
	...
	printf ("%s: command not found.", cmd->command[0]);
	exit (0);
	return (0);
}
...
void    execute_two_electric_boogaloo(t_minishell *shell)
{
	...
	i = 0;
	// if (check_builtin(cmd, shell) == 0) // segf if empty input
		// return ;
	// child = fork();
	// pipe(tunnel);
	// if (child == 0)
